...
public class CommentServiceImpl implements CommentService {
    ...
    public CommentDto updateComment(UpdateCommentRequest updateCommentRequest) {
        Comment comment = commentRepository.findById(updateCommentRequest.commentId())
                .orElseThrow(() -> new ResourceNotFound("Comment", "commentId", updateCommentRequest.commentId().toString()));

        User user = userRepository.findById(updateCommentRequest.userId())
                .orElseThrow(() -> new ResourceNotFound("User", "userId", updateCommentRequest.userId().toString()));

        if (!comment.getUser().getUserId().equals(user.getUserId())) {
            throw new RuntimeException("You are not authorized to update this comment.");
        }

        comment.setCommentText(updateCommentRequest.commentText());
    ...
    @Override
    public CommentResponse getAllCommentsByPost(Long postId, Integer pageNumber, Integer pageSize, String sortBy, String sortDir) {
        Post post = postRepository.findById(postId)
                .orElseThrow(()-> new ResourceNotFound("Post", "postId", postId.toString()));
        ...
        return new CommentResponse(all_comments,
    ...
    @Override
    public CommentResponse getAllCommentsByUser(Long userId, Integer pageNumber, Integer pageSize, String sortBy, String sortDir) {
        User user = userRepository.findById(userId)
                .orElseThrow(()-> new ResourceNotFound("User", "userId", userId.toString()));
        Sort sort = sortDir.equalsIgnoreCase("asc") ? Sort.by(sortBy).ascending() : Sort.by(sortBy).descending();
        Pageable pageable = PageRequest.of(pageNumber, pageSize, sort);
