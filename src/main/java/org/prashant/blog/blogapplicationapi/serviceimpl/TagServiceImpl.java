...
public class TagServiceImpl implements TagService{
    ...
    public TagDto createTag(CreateTagRequest createTagRequest) {
        Tag tag = new Tag();
        tag.setTagName(createTagRequest.tagName());
    ...
    public TagResponse getAllTags(Integer pageNumber, Integer pageSize, String sortBy, String sortDir) {
        ...
        List<TagDto> tagDtos = page_tags.getContent().stream().map(tag -> modelMapper.map(tag, TagDto.class)).toList();
        return new TagResponse(tagDtos,
                page_tags.getNumber(),
                page_tags.getSize(),
